...
Latest updates: hps://dl.acm.org/doi/10.1145/3287560.3287596

PDF Download
3287560.3287596.pdf
31 March 2026
Total Citations: 1422
Total Downloads: 16682

Citation in BibTeX format

MARGARET MITCHELL

FAT* '19: Conference on Fairness,
...
hps://doi.org/10.1145/3287560.3287596
ISBN: 9781450361255
...
high-impact tasks in areas such as law enforcement, medicine, education, and employment. In order to clarify the intended use cases
...
models be accompanied by documentation detailing their performance characteristics. In this paper, we propose a framework that
...
of conditions, such as across different cultural, demographic, or phenotypic groups (e.g., race, geographic location, sex, Fitzpatrick skin
...
are intended to be used, details of the performance evaluation procedures, and other relevant information. While we focus primarily
...
model. To solidify the concept, we provide cards for two supervised models: One trained to detect smiling faces in images, and
...
CCS CONCEPTS
• General and reference → Evaluation; • Social and professional topics → User characteristics; • Software and its engineering → Use cases; Documentation; Software evolution; • Humancentered computing → Walkthrough evaluations;
...
Researchers have discovered systematic biases in commercial machine learning models used for face detection and tracking [4, 9, 49],
attribute detection [5], criminal justice [10], toxic comment detection [11], and other applications. However, these systematic errors
...
face recognition systems failed to detect her face [4], she collaborated with other researchers to demonstrate the disproportionate
...
supplied) provide very little information regarding model performance characteristics, intended use cases, potential pitfalls, or other
information to help users evaluate the suitability of these systems
to their context. This highlights the need to have detailed documentation accompanying trained machine learning models, including
...
we call model cards. Model cards (for model reporting) are complements to “Datasheets for Datasets” [21] and similar recently
...
General Motors in 1976, claiming that the company’s hiring practices discriminated against Black women. In their court opinion,
...
importance of intersectional analyses: empirical analyses that emphasize the interaction between various demographic categories
...
benchmarking various systems under different conditions. For example, as noted in [21], the electronic hardware industry provides
datasheets with detailed characterizations of components’ performances under different test conditions. By contrast, despite the
...
communicating characteristics of datasets used in machine learning [3, 21, 28] to help users understand the context in which the
datasets should be used. We focus on the complementary task for
machine learning models, proposing a standardized method to evaluate the performance of human-centric models: Disaggregated by
...
Outside of machine learning, the need for population-based reporting of outcomes as suggested here has become increasingly evident. For example, in vehicular crash tests, dummies with prototypical female characteristics were only introduced after researchers
discovered that women were more likely than men to suffer serious head injuries in real-world side impacts [18]. Similarly, drugs
...
U.S. Food and Drug Administration mandated that clinical trial results be disaggregated by groups such as age, race and gender [16].
While population-based analyses of errors and successes can be
provided for unitary groups such as “men”, “women”, and “nonbinary” gender groups, they should also be provided intersectionally, looking at two or more characteristics such as gender and age
simultaneously. Intersectional analyses are linked to intersectionality theory, which describes how discrete experiences associated
...
understanding the systematic impacts of these models before deploying them.
...
candidate models for deployment across not only traditional evaluation metrics but also along the axes of ethical, inclusive, and fair
considerations. This goes further than current solutions to aid stakeholders in different contexts. For example, to aid policy makers and
...
model’s predictions can inform their design and implementation decisions.
...
• ML-knowledgeable individuals can be informed on different options for fine-tuning, model combination, or additional
...
help to standardize decision making processes for invested stakeholders, but it also encourages forward-looking model analysis
techniques. For example, slicing the evaluation across groups functions to highlight errors that may fall disproportionately on some
groups of people, and accords with many recent notions of mathematical fairness (discussed further in the example model card in
Figure 2). Including group analysis as part of the reporting procedure prepares stakeholders to begin to gauge the fairness and
inclusion of future outcomes of the machine learning system. Thus,
in addition to supporting decision-making processes for determining the suitability of a given machine learning model in a particular
...
The proposed set of sections below are intended to provide relevant details to consider, but are not intended to be complete or
exhaustive, and may be tailored depending on the model, context,
and stakeholders. Additional details may include, for example, interpretability approaches, such as saliency maps, TCAV [33], and
Path-Integrated Gradients [38, 43]); stakeholder-relevant explanations (e.g., informed by a careful consideration of philosophical,

Figure 1: Summary of model card sections and suggested
prompts for each.
psychological, and other factors concerning what is as a good explanation in different contexts [22]); and privacy approaches used
...
Feedback on the model: E.g., what is an email address that people
...
For human-centric computer vision models, the visual presentation of age, gender, and Fitzpatrick skin type [15] may be relevant.
...
Details pertaining to groups, including who annotated the training and evaluation datasets, instructions and compensation given
...
should not be seen as a requirement to compromise private information or reveal proprietary training techniques; rather, a place to
disclose basic decisions and facts about the model that the organization can share with the broader community in order to better
...
Out-of-scope uses: Here, the model card should highlight technology that the model might easily be confused with, or related
...
techniques, and background blurring for portrait mode. Performance may also vary across real or simulated traditional camera
settings such as aperture, shutter speed and ISO. Similarly, video
and audio input will be dependent on the choice of recording instruments and their settings.
4.3.3 Environment. A further factor affecting model performance
is the environment in which it is deployed. For example, face detection systems are often less accurate under low lighting conditions or
...
Relevant factors: What are foreseeable salient factors for which
...
necessary to make more informed decisions about model development. A model card could even extend beyond these summary
statistics to reveal other measures of differences between distributions such as cross entropy, perplexity, KL divergence and pinned
...
translation system could compare BLEU scores [40] across demographic groups, and a model card for a speech recognition system
could compare word-error rates. Although the primary outputs of
these systems are not scores, looking at the score differences between populations may yield meaningful insights since comparing
raw inputs quickly grows too complex.

Metrics

The appropriate metrics to feature in a model card depend on the
type of model that is being tested. For example, classification systems in which the primary output is a class label differ significantly
from systems whose primary output is a score. In all cases, the
reported metrics should be determined based on the model’s structure and intended use. Details for this section include:
Model performance measures: What measures of model performance are being reported, and why were they selected over other
...
decision thresholds.
Approaches to uncertainty and variability: How are the measurements and estimations of these metrics calculated? For example, this may include standard deviation, variance, confidence
intervals, or KL divergence. Details of how these values are approximated should also be included (e.g., average of 5 runs, 10-fold
cross-validation).
...
makes it especially important to understand the confidence intervals for the reported metrics. Confidence intervals for metrics derived from confusion matrices can be calculated by treating the
...
Evaluation datasets should include datasets that are publicly available for third-party use. These could be existing datasets or new
...
To ensure that model cards are statistically accurate and verifiable, the evaluation datasets should not only be representative
of the model’s typical use cases but also anticipated test scenarios and challenging cases. For instance, if a model is intended for
...
on two evaluation sets: one that matches the workplace’s population, and another set that contains individuals that might be more
...
outside of the initial domain used in training. In some of these situations, synthetically generated datasets may provide representation
...
4.4.2 Score-based analyses. For score-based systems such as pricing models and risk assessment algorithms, describing differences
in the distribution of measured metrics across groups may be helpful. For example, reporting measures of central tendency such as
...
represented in the evaluation dataset? Are there additional recommendations for model use? What are the ideal characteristics of an
...
that automatically finds ‘fun moments’ in images). Additional finetuning, for example, with images of older men, may help create a
...
different metrics across disaggregated population subgroups corresponds to how fairness is often defined [37, 48]. Quantitative
...
TOXICITY classifier built to detect ‘toxicity’ in text [32], and is presented in Figure 3. To evaluate the model, we use an intersectional
...
performance for several terms, especially “lesbian”, “gay”, and “homosexual”. This is consistent with what some users of the initial
TOXICITY model found, as reported by the team behind Perspective
...
We have proposed frameworks called model cards for reporting information about what a trained machine learning model is and how
...
Factors
• Based on known problems with computer vision face technology, potential relevant factors include groups for gender, age, race, and Fitzpatrick skin type;
...
threshold-agnostic separability of toxic and non-toxic comments for each group, within the context of a background
distribution of other groups.
Ethical Considerations
• Following [31], the Perspective API uses a set of values
to guide their work. These values are Community, Transparency, Inclusivity, Privacy, and Topic-neutrality. Because
of privacy considerations, the model does not take into account user history when making judgments about toxicity.
...
Evaluation Data
• A synthetic test set generated using a template-based approach, as suggested in [11], where identity terms are
...
cards are intended to accompany a model after careful review has
determined that the foreseeable benefits outweigh the foreseeable
risks in the model’s use or release.
To demonstrate the use of model cards in practice, we have provided two examples: A model card for a smiling classifier tested
on the CelebA dataset, and a model card for a public toxicity detector tested on the Identity Phrase Templates dataset. We report
...
to be applicable across different institutions, contexts, and stakeholders. It also is suitable for recently proposed requirements for
analysis of algorithmic decision systems in critical social institutions, for example, for models used in determining government
...
and specificity in order to accommodate the wide variety of machine learning model types and potential use cases. Therefore the
...
degree needed to prevent misleading representations of model results (whether intended or unintended). It is therefore important to
...
(both quantitative and qualitative), “adversarial testing” by technical and non-technical analysts, and more inclusive user feedback
mechanisms. Future work will aim to refine the methodology of
creating model cards by studying how model information is interpreted and used by different stakeholders. Researchers should also
explore how model cards can strengthen and complement other
transparency methods

[7] Federal Trade Commission. 2016. Big Data: A Tool for Inclusion or Exclusion? Understanding the Issues. (2016). https://www.ftc.gov/reports/
...
[26] Moritz Hardt, Eric Price, , and Nati Srebro. 2016. Equality of Opportunity in Supervised Learning. In Advances in Neural Information Processing
...
[28] Sarah Holland, Ahmed Hosny, Sarah Newman, Joshua Joseph, and Kasia Chmielinski. 2018. The Dataset Nutrition Label: A Framework To Drive Higher Data
...
[35] Der-Chiang Li, Susan C Hu, Liang-Sian Lin, and Chun-Wu Yeh. 2017. Detecting representative data and generating synthetic samples to improve learning
...
Disparities in Commercial Gender Classification. In Proceedings of the 1st Conference on Fairness, Accountability and Transparency (Proceedings of Machine Learning Research), Sorelle A. Friedler and Christo Wilson (Eds.), Vol. 81. PMLR, New
...
Vision (ICCV).
[37] Shira Mitchell, Eric Potash, and Solon Barocas. 2018. Prediction-Based Decisions and Fairness: A Catalogue of Choices, Assumptions, and Definitions.
...
Gaze of AI. (2018).
[42] Microsoft Research. 2018. Project InnerEye - Medical Imaging AI to Empower Clinicians. (2018). https://www.microsoft.com/en-us/research/project/
...
2018.
Unintended bias and names of frequently targeted
groups.
Medium (2018).
...
[51] Mingyuan Zhou, Haiting Lin, S Susan Young, and Jingyi Yu. 2018. Hybrid sensing face detection and registration for low-light and unconstrained conditions.
